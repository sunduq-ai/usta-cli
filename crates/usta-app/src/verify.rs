...
use thiserror::Error;
use usta_core::snapshot::{ManagedLock, ManagedLockParseError};
use usta_ports::fs::{FileSystem, FsError};

use crate::scaffold::snapshot::LOCK_PATH;

/// Where the lock and snapshot files live, mirrored from `scaffold::snapshot`.
pub use crate::scaffold::snapshot::SNAPSHOT_PATH;
...
mod tests {
    ...
    /// In-memory FS for app-layer tests (we can't import `usta-adapters` here).
    #[derive(Default)]
    struct Mem {
