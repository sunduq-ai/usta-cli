...
|------:|:------:|---------|
| **P10** — Close the scaffold loop (v0.4.0) | ✅ done | `usta new` now runs post-scaffold actions by default: `git init -b main` + initial commit, and dependency install via detected package managers (`pnpm`/`npm`/`uv`/`go`). Wires the previously-defined `VcsClient` + `PackageManager` ports with new `GitCli` + `CliPackageManager` adapters. Ecosystem detection is a pure, unit-tested `classify` fn feeding a bounded directory walk; install runs **before** the commit so lockfiles are captured and the working tree is clean. Best-effort throughout (missing tool → note, failure → warning, never fails the scaffold; never clobbers an existing repo). The formerly-inert `--no-git`/`--no-install` flags are now real. Validated in sandbox across hello-world (npm), nx-monorepo (pnpm + uv), and a restricted-PATH degradation run. **172 tests pass.** |
| **P9** — v0.3.0: usable out of the box | ✅ done | Dogfooded the published binary as a fresh user and fixed what broke. **Embedded the built-in templates** via `include_dir` so `cargo install usta && usta new --template nx-monorepo` works with no `--templates-dir` (previously dead-ended — templates shipped in the tarball but weren't installed beside the binary); extracted to a per-version cache dir on first use. Added **"did you mean?"** suggestions for mistyped template/feature ids. Fixed two hello-world template bugs found by actually *running* the output (`index.js` shipped a literal `{{ project_name }}`; `with-router` injected a `require` for a `router.js` it never shipped). Centralized 4 duplicated templates-dir resolvers into `wiring::resolve_templates_dir`. **162 tests pass**. |
